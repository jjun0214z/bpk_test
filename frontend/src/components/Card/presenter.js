...
const CardUi = props => {
  return (
    <div
      className={
        props.type === "vertical"
          ? classNames(styles.card, styles.vertical)
          : classNames(styles.card, styles.horizontal)
      }
    >
      <div className={styles.photo}>
        <img
          src={props.image.file && require("images/img_card.png")}
        ...
        {props.grade && (
          <div className={classNames(styles.column, styles.bottom)}>
            <div className={styles.grade}>
              {props.grade.map((grade, idx) => (
                <Ratings grade={grade} key={idx} />
              ))}
            </div>
            {props.content && props.type === "vertical" && (
              <p className={styles.text}>{props.content}</p>
            )}
          </div>
        )}
